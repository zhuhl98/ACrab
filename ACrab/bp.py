...
class BehaviorPretraining(nn.Module):
    ...
    def __init__(self, *,
        ...
        """
            Inputs:
                ...
                w: An nn.module of importance weight w that measures the ratio of occupancy distribution of the target policy to behavior policy
    ...
    def compute_w_loss(self, observations, actions, next_observations, rewards, terminals, **kwargs):
        w1, w2 = self.w(observations, actions)
        w_loss = 0.
        # Q Loss with TD error and/or Residual Error using Target Q
        def compute_bellman_backup(v_next):
            assert rewards.shape == v_next.shape
        ...
        if self.td_weight>0:
            ...
            for qs in qs_all:
                w_loss += (torch.abs(torch.mean(torch.mul(w1, torch.sub(td_targets, qs.detach())))) - self.w_a * torch.mean(torch.square(w1-1))/2 - torch.square(torch.mean(w1)-1)/2) * self.td_weight
                w_loss += (torch.abs(torch.mean(torch.mul(w2, torch.sub(td_targets, qs.detach())))) - self.w_a * torch.mean(torch.square(w2-1))/2 - torch.square(torch.mean(w2)-1)/2) * self.td_weight
        # Residual error
        if self.rs_weight>0:
            # next_qs_all = self.qf.both(next_observations, next_policy_actions)  # inference
            for qs, next_qs in zip(qs_all, next_qs_all):
                rs_targets = compute_bellman_backup(next_qs)
                rs_targets = rs_targets.detach()
                w_loss += (torch.abs(torch.mean(torch.mul(w1, torch.sub(rs_targets, qs.detach())))) - self.w_a * torch.mean(torch.square(w1-1))/2 - torch.square(torch.mean(w1)-1)/2) * self.rs_weight 
                w_loss += (torch.abs(torch.mean(torch.mul(w2, torch.sub(rs_targets, qs.detach())))) - self.w_a * torch.mean(torch.square(w2-1))/2 - torch.square(torch.mean(w2)-2)/2) * self.rs_weight
        w_loss = -w_loss
        # Log
        info_dict = {"w loss": w_loss.item(),
                     "Average w value": w2.mean().item()}
        return w_loss, info_dict
    ...
    def update(self, **batch):
        qf_loss = vf_loss = policy_loss = w_loss = torch.tensor(0., device=batch['observations'].device)
        ...
        if self.w is not None and self.reg != 'abs' and self.reg != 'wapprox':
            w_loss, w_info_dict = self.compute_w_loss(**batch)
        # Update
        loss = policy_loss + qf_loss + vf_loss + w_loss
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
        # Log
        if self.reg != 'abs' and self.reg != 'wapprox':
            info_dict = {**qf_info_dict, **vf_info_dict, **policy_info_dict, **w_info_dict}
        else:
