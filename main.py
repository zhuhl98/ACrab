...
from ACrab.policy import GaussianPolicy
from ACrab.value_functions import TwinQ, TwinW, ValueFunction
from ACrab.util import Log, set_seed
from ACrab.bp import BehaviorPretraining
from ACrab.acrab import ACrab
from ACrab.util import evaluate_policy, sample_batch, traj_data_to_qlearning_data, tuple_to_traj_data, DEFAULT_DEVICE
...
def main(args):
    ...
    # Setup logger
    log_path = Path(args.log_dir) / args.env_name / ('_beta' + str(args.beta)) / ('reg_' + args.reg + '_C_infty_' + str(args.C_infty) + '_w_frac_' + str(args.w_frac))
    log = Log(log_path, vars(args))
    log(f'Log dir: {log.dir} device: {DEFAULT_DEVICE}')
    ...
    w = TwinW(obs_dim, act_dim, hidden_dim=args.hidden_dim, n_hidden=args.n_hidden, w_l_2_bound=args.w_l_2_bound, w_l_infty_bound=args.w_l_infty_bound).to(DEFAULT_DEVICE)
    ...
    rl = ACrab(
        ...
        policy_lr=args.slow_lr*args.lr_multiplier,
        qf_lr=args.fast_lr*args.lr_multiplier,
        w_lr=args.w_lr*args.lr_multiplier,
        w_rs=args.w_rs,
        n_steps_w_opt=args.n_steps_w_opt,
        reg=args.reg,
        w_frac=args.w_frac,
        n_w=args.n_w,
        w_a=args.w_a,
        f_a=args.f_a,
        C_infty=args.C_infty,
    ...
    bp = BehaviorPretraining(qf=qf, target_qf=target_qf, policy=policy, w=w, lr=args.fast_lr, discount=args.discount,
                             td_weight=0.5, rs_weight=0.5, fixed_alpha=None, action_shape=act_dim,
                             Vmin=Vmin, Vmax=Vmax, w_a=args.w_a, f_a=args.f_a, reg=args.reg,).to(DEFAULT_DEVICE)
    ...
    for step in trange(args.n_steps):
        train_metrics = rl.update(log_func=log, upd_idx=step, **sample_batch(dataset, args.batch_size))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--w_lr', type=float, default=5e-4)
    parser.add_argument('--n_steps_w_opt', type=int, default=1)
    parser.add_argument('--fast_lr', type=float, default=5e-4)
    parser.add_argument('--slow_lr', type=float, default=5e-7)
    parser.add_argument('--w_l_2_bound', type=float,default=50)
    parser.add_argument('--w_l_infty_bound', type=float,default=100)
    parser.add_argument('--beta', type=float, default=2.0)
    parser.add_argument('--C_infty', type=float, default=1)
    parser.add_argument('--w_rs', type=float, default=0.5)
    ...
    parser.add_argument('--w_frac',type=float, default=0.5)
    parser.add_argument('--reg', type=str, default='wapprox')
    parser.add_argument('--clip_v', action='store_true')
    parser.add_argument('--w_a', type=float, default=0.05)
    parser.add_argument('--f_a',type=float,default=0)
